...
def get_lyrics(id):
    response = requests.get("http://api.musixmatch.com/ws/1.1/track.lyrics.get?track_id=" + id + "&apikey=" + API_KEY)
...
def analyze_tones(lyrics):
    ...
    sentences = filter(lambda x: len(x['tones']) > 0, response.json()['sentences_tone'])

    tones = {}

    for sentence in sentences:
        for tone in sentence['tones']:
            if not tone['tone_id'] in tones:
                tones[tone['tone_id']] = {
                    'avg': tone['score'],
                    'count': 1
                }
...
def analyze_personality(lyrics):
    ...
    response = requests.post('https://gateway.watsonplatform.net/personality-insights/api/v3/profile?version=2017-09-21',
